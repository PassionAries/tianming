...
const truces = {};
for (let i = 0; i < 1000; i++) truces[`势力${i}|势力${i + 1}`] = 50000;
const world = {
  turn: 10,
  _campaignId: 'truce-hot-world',
  _timelineId: 'truce-hot-timeline',
...
assert.strictEqual(world._warTruces.truces, world._warTruces.truces, 'hot lookup should preserve the normalized dictionary identity');
assert.notStrictEqual(world._warTruces.truces, originalDictionary, 'the first legacy read should normalize into one safe dictionary');
...
console.log('smoke-war-truce-hot-path ok (1000 truces / 10000 lookups)');
