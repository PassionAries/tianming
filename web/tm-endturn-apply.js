...
(function(global) {
  ...
  ns.writeBack = async function(ctx) {
      ...
      if(p1){
        ...
        if (p1.char_updates && Array.isArray(p1.char_updates)) {
          ...
          p1.char_updates.forEach(function(cu) {
            ...
            var _wgFlat = global.TM && global.TM.AIChange && global.TM.AIChange.WriteGuards;
            if (cu.new_stance && typeof cu.new_stance === 'string' && (!(_wgFlat && _wgFlat.sensitiveCharFieldSourced) || _wgFlat.sensitiveCharFieldSourced(GM, p1, ch, 'stance', ch.name))) {
              ch.stance = cu.new_stance;
            }
            // 党派变化
            if (cu.new_party !== undefined && (!(_wgFlat && _wgFlat.sensitiveCharFieldSourced) || _wgFlat.sensitiveCharFieldSourced(GM, p1, ch, 'party', ch.name))) {
    ...
    return ns.stages._applyPostValidateAssemble(ctx, { p1: p1, shizhengji: shizhengji, zhengwen: zhengwen, playerStatus: playerStatus, playerInner: playerInner, turnSummary: turnSummary, shiluText: shiluText, szjTitle: szjTitle, szjSummary: szjSummary, personnelChanges: personnelChanges, hourenXishuo: hourenXishuo, _applied: _applied, _applyStart: _applyStart });
  };
