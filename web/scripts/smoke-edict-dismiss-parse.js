...
console.log('===== M·三审实现级修正:单字改误伤 / 形态a极性 / 唯一名单 / 保守契约 =====');
(function () {
  const ctx = fixtureCtx();
  // ①单字"改"仅紧邻动词才承接:"改任/改元"在片段内任意位置不翻正(对照"转任"版本一致不摘)
  assert(ctx.extractEdictActions('切勿因改任他职之议而查办陈奇瑜').dismissals.length === 0, 'M①·「切勿因改任…查办陈奇瑜」单字改不翻正·不摘');
  assert(ctx.extractEdictActions('不得因改元传闻而查办陈奇瑜').dismissals.length === 0, 'M①·「不得因改元…查办陈奇瑜」不摘');
  assert(ctx.extractEdictActions('切勿因转任他职之议而查办陈奇瑜').dismissals.length === 0, 'M①对照·「转任」版本同样不摘(极性一致)');
  // ②形态a承接前查被承接动词极性:改为+否定→不承接(与无标点单片段同语义一致)
  assert(ctx.extractEdictActions('赦免陈奇瑜死罪，改为不要革职留用').dismissals.length === 0, 'M②·「改为不要革职」负极性·不承接');
  assert(ctx.extractEdictActions('赦免陈奇瑜死罪，改为暂缓革职留用').dismissals.length === 0, 'M②·「改为暂缓革职」负极性·不承接');
  assert(dismissed(ctx.extractEdictActions('赦免陈奇瑜死罪，改革职留用'), '陈奇瑜'), 'M②对照·「改革职」正极性·仍承接摘');
  // ③"唯一名"=去重后 canonical 严格==1 才缝·名单含多人一概不缝(跨段+无标点两变体)
  assert(ctx.extractEdictActions('赦免陈奇瑜、王在晋死罪，改革职留用').dismissals.length === 0, 'M③·多人列举承接·两人都不入 dismissals(跨段)');
  assert(ctx.extractEdictActions('赦免陈奇瑜、王在晋死罪改革职留用').dismissals.length === 0, 'M③·多人列举承接·两人都不入 dismissals(无标点单片段)');
  // ④保守契约现状固化:"革职 陈奇瑜等人"名后有"等人"→不缝(该片段无动词·退回枚举扫描·不摘)
  assert(ctx.extractEdictActions('革职 陈奇瑜等人').dismissals.length === 0, 'M④·保守契约:「革职␣陈奇瑜等人」名后有谓词不缝(现状固化)');
})();
