...
/* ===================== 立绘字形 / SVG 基件 ===================== */
function faceHtml(p,cls){ var _pu=p.portrait||(window.TM&&TM.WorkshopAssets?TM.WorkshopAssets.portraitFor(p.name):''); /* 批Ⅴ·工坊立绘包按同名兜底(角色自带立绘优先) */ if(_pu){return '<img loading="lazy" src="'+esc(_pu)+'" alt="" onerror="this.style.display=\'none\';this.parentNode.classList.add(\'fallback\')">';} return '<span class="glyph">'+esc((p.name||'?').charAt(0))+'</span>'; }
