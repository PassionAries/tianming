...
(function(){
  ...
  function renderDiscover() {
    var packs = (state.catalog && state.catalog.packs) || [];
    var catalogReady = state.catalogStatus === 'ok' || state.catalogStatus === 'fallback';
    ...
    if (!catalogReady) {
      return heroHtml + truthEmpty('检', '目录来源不可用', state.catalogError || '正式目录与仓库清单均未能读取。', '<button class="btn primary sm" onclick="TMContentManager.refreshWorkshopSources()">重试真源</button>');
    }
    ...
    return heroHtml + (state.catalogStatus === 'fallback' ? '<div class="status atelier-fallback-note">正式在线目录不可用；当前只展示仓库内置官方清单。</div>' : '') +
      mallTypeChips() +
